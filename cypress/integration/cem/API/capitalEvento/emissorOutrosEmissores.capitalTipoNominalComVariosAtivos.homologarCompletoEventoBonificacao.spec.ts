...
let validarValorCapital: number
...
describe('Emissor Outros Emissores - Homologar completo evento Somar Quantidade dos Ativos - Capital Tipo Nominal', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('PUT - Homologar completo evento Somar Quantidade dos Ativos', () => {
        ...
        it('GET - Consultar capital tipo nominal com vários ativos após homologar completo evento Somar Quantidade dos Ativos', () => {
            ...
            validarValorCapital = valorPrecoCapital + valorPrecoCapital
            ...
            cy.getCEM('Capital/get-active/' + idEmissor).then(resp => {
                ...
                assert.isTrue(resp.body.approvalDate.includes(dataHoje), "Data aprovacao capital")
                expect(resp.body).to.have.property('capitalEventCode', idEventoCapital)
                expect(resp.body).to.have.property('capitalPriceValue', validarValorCapital)
                expect(resp.body).to.have.property("capitalStatusCode", 0)
                expect(resp.body).to.have.property('issuerCode', idEmissor)
        ...
        it('GET Histórico capital - Consultar no histórico os eventos capital já homologado', () => {
            ...
            cy.getCEM('Capital/' + idEmissor + '/history').then(resp => {
                ...
                assert.isTrue(buscaEventoSomarQtdeAtivos.some(i => i.capital == validarValorCapital), "valor capital - Bonificação")
