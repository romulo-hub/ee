...
describe('Emissor Cia Estrangeira - Homologar parcial evento Conversão de ações com manter restante ativado - Capital Tipo Unitário', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('GET Histórico capital - Consultar no histórico o capital antes de homologar parcial o evento Conversão de ações', () => {
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital cadastrado pendente de homologação', () => {
            ...
            cy.getCEM('capitalEvent/' + idEmissor + '/homologate-list').then(resp => {
                ...
                expect(resp.body[0].capitalType).to.have.property('id', 5)
        ...
        it('GET - Consultar capital tipo unitário com um ativo após homologar parcial evento Conversão de ações', () => {
        ...
        it('GET Histórico capital - Consultar no histórico os eventos capital já homologado', () => {
            ...
            cy.getCEM('Capital/' + idEmissor + '/history').then(resp => {
                ...
                assert.isTrue(buscaEventoConversaoAcoes.some(i => i.quantity == qtdesAtivosPreferencialParcial), "Qtdes ativo preferencial capital - Conversão de ações")
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital restante após homologação parcial', () => {
            ...
            cy.getCEM('capitalEvent/' + idEmissor + '/homologate-list').then(resp => {
                ...
                idListaEventoAtivoPreferencialCapital = resp.body[0].capitalEventShares[0].id
                ...
                expect(resp.body[0].capitalEventShares[0]).to.have.property('capitalShareRedemptionIndicator', resgatavelPreferencialAtivosCapital)
                expect(resp.body[0].capitalEventShares[0]).to.have.property('shareClassQuantity', qtdeAtivoPreferencialRestanteEventoCapital)
                expect(resp.body[0].capitalEventShares[0]).to.have.property('tagAlongPercentage', tagAlongPreferencialAtivosCapital)
                expect(resp.body[0].capitalType).to.have.property('actionCode', 1)
                expect(resp.body[0].capitalType).to.have.property('description', "Conversão de ações")
                expect(resp.body[0].capitalType).to.have.property('fieldsCode', 1)
