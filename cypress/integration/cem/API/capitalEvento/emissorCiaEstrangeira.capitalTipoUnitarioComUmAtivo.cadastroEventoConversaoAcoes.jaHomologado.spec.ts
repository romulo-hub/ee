...
import { EnumTipoEventoCapital, EnumTipoPrecoCapital, EnumApi, EnumTipoClasseAtivo } from '../../utilitarios/enum.utilitario'
...
let idEventoNaoExiste: number
let valorUnitarioAposEventoHomologado: number

describe('Emissor Cia Estrangeira - Cadastro evento Conversão de ações já homologado - Capital Tipo Unitário', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('POST - Cadastrar evento conversão ações já homologado', () => {

            statusEventoCapital = true
            qtdeAtivoPreferencialAlterar = 1860
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital cadastrado já homologado na lista de eventos pendente de homologação', () => {
        ...
        it('PUT - Editar evento capital que já foi homologado', () => {

            cy.getCEM('capitalEvent/' + idEventoCapital).then(resp => {

                cy.log(resp.body)

                let quantidadeAtivosEventoCapital = 2.252
                        ...
                        "capitalEventShares": [
                            {
                                "shareClassTypeCode": idTipoAtivoPreferencialCapital,
                                "capitalShareRedemptionIndicator": resgatavelPreferencialAtivosCapital,
                                "shareClassQuantity": quantidadeAtivosEventoCapital,
                                "tagAlongPercentage": tagAlongPreferencialAtivosCapital
                            }
                        ]
                    }).then(resp => {

                        expect(resp.status).to.be.eq(EnumApi.conflict)
                        assert.isTrue(resp.body.description.includes("Somente eventos com status pendente podem ser alterados! O status do evento é Approved"), "Evento Capital com status Aprovado")
        ...
        it('GET - Consultar capital tipo unitário com um ativo após cadastrar evento Conversão de ações já homologado', function () {
            ...
            valorUnitarioAposEventoHomologado = 15.3225806
        ...
        it('DELETE - Evento capital cadastrado já homologado', () => {
            ...
            cy.deleteCEM('capitalEvent', idEventoCapital).then(resp => {
                ...
                expect(resp.status).to.be.eq(EnumApi.conflict)
                assert.isTrue(resp.body.description.includes("Não é permitido excluir esse evento, porque já foi homologado"), "Evento capital já homologado não pode excluir")
        ...
        it('GET - Consultar evento capital já homologado não foi excluido', () => {
        ...
        it('DELETE - Evento capital que não existe', () => {

            idEventoNaoExiste = 987654

            cy.deleteCEM('capitalEvent', idEventoNaoExiste).then(resp => {

                expect(resp.status).to.be.eq(EnumApi.notFound)
                assert.isTrue(resp.body.description.includes("Evento não encontrado"), "evento capital não existe")
        ...
        it('GET - Consultar evento capital que não existe', () => {

            idEventoNaoExiste = 987654

            cy.getCEM('capitalEvent/' + idEventoNaoExiste).then(resp => {

                expect(resp.status).to.be.eq(EnumApi.notFound)
                assert.isTrue(resp.body.detail.includes("CapitalEvent_GetById"), "Evento capital não existe")
        ...
        it('GET - Lista comportamento do tipo evento capital', () => {

            cy.getCEM('capitalEvent/get-action-field-list/').then(resp => {
        ...
        it('GET - Lista comportamento', () => {

            cy.getCEM('capitalEvent/get-action-list').then(resp => {
        ...
        it('GET - Lista status do evento capital', () => {

            cy.getCEM('capitalEvent/get-status-list').then(resp => {
