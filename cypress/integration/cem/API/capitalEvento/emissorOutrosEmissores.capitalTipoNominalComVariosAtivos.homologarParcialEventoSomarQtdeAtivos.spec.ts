...
let valorCapitalHomolg: number
let sumCapitalValue:number

describe('Emissor Outros Emissores - Homologar parcial evento Bonificação com manter restante ativado - Capital Tipo Nominal', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('GET Histórico capital - Consultar no histórico o capital antes de homologar parcial o evento Bonificação', () => {
        ...
        it('POST - Cadastrar evento Bonificação pendente de homologação', () => {
        ...
        it('PUT - Homologar parcial evento Bonificação', () => {

            qtdesAtivosPreferencialParcial = qtdeAtivoPreferencialAlterar - 600
            qtdesAtivosOrdinarioParcial = qtdeAtivoOrdinarioAlterar - 800
            ...
            valorCapitalHomolg = valorPrecoCapital - 100
            ...
            cy.putCEM('capitalEvent/homolog',
                {
                    ...
                    "id": idEventoCapital,
                    "capitalPriceValue": {
                        "homologated": valorCapitalHomolg,
                        "keepPending": true
                    },
        ...
        it('GET - Consultar evento capital que foi homologado parcial', () => {
            ...
            cy.getCEM('capitalEvent/' + idEventoCapital).then(resp => {
                ...
                expect(resp.body).to.have.property('capitalPriceValue', valorCapitalHomolg)
                expect(resp.body).to.have.property('issuerCode', idEmissor)
        ...
        it('GET - Consultar capital tipo nominal com vários ativos após homologar completo evento Bonificação', () => {

            qtdeTotalAtivoOrdinarioAposEventoHomologado = qtdeAtivosOrdinarioCapital + qtdesAtivosOrdinarioParcial
            qtdeTotalAtivoPreferencialAposEventoHomologado = qtdeAtivosPreferencialCapital + qtdesAtivosPreferencialParcial
            qtdeTotalAtivosAposEventoHomologado = qtdeAtivosOrdinarioCapital + qtdesAtivosOrdinarioParcial + qtdeAtivosPreferencialCapital + qtdesAtivosPreferencialParcial
            idNovoCapitalAposEventoHomologado = idCapital + 1

            sumCapitalValue = valorCapitalHomolg + valorPrecoCapital
            ...
            cy.getCEM('Capital/get-active/' + idEmissor).then(resp => {
                ...
                assert.isTrue(resp.body.approvalDate.includes(dataHoje), "Data aprovacao capital")
                expect(resp.body).to.have.property('capitalEventCode', idEventoCapital)
                expect(resp.body).to.have.property('capitalPriceValue', sumCapitalValue)
                expect(resp.body).to.have.property("capitalStatusCode", 0)
                expect(resp.body).to.have.property('issuerCode', idEmissor)
        ...
        it('GET Histórico capital - Consultar no histórico os eventos capital já homologado', () => {
            ...
            cy.getCEM('Capital/' + idEmissor + '/history').then(resp => {
                ...
                assert.isTrue(buscaEventoSomarQtdeAtivos.some(i => i.capital == sumCapitalValue), "valor capital - Bonificação")
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital restante após homologação parcial', () => {
            ...
            cy.getCEM('capitalEvent/' + idEmissor + '/homologate-list').then(resp => {
                ...
                let remainingPriceValue = valorPrecoCapital - valorCapitalHomolg

                expect(resp.status).to.have.equal(EnumApi.success)
                expect(resp.body[0]).to.have.property('id', idEventoRestanteCapital)
                expect(resp.body[0]).to.have.property('capitalPriceValue', remainingPriceValue)
                expect(resp.body[0]).to.have.property('issuerCode', idEmissor)
                expect(resp.body[0]).to.have.property('capitalEventTypeCode', idTipoEventoCapital)
                expect(resp.body[0].capitalEventStatus).to.be.eq(0)
                expect(resp.body[0]).to.have.property("capitalEventTypeDescription", 'Bonificação')
