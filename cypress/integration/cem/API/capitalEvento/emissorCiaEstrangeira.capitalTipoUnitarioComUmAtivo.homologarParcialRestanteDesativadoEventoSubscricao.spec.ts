...
describe('Emissor Cia Estrangeira - Homologar parcial o evento Subscrição com o restante desativado - Capital Tipo Unitário', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('GET Histórico capital - Consultar no histórico o capital antes de homologar parcial o evento Subscrição', () => {
        ...
        it('POST - Cadastrar evento Subscrição pendente de homologação', () => {

            valorEventoCapitalAlterar = 2450
            qtdeAtivoAlterar = 1822
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital cadastrado pendente de homologação', () => {
            ...
            cy.getCEM('capitalEvent/' + idEmissor + '/homologate-list').then(resp => {
                ...
                expect(resp.body[0]).to.have.property("capitalEventTypeDescription", 'Subscrição')
                ...
                expect(resp.body[0].capitalType).to.have.property("description", 'Subscrição')
        ...
        it('PUT - Homologar parcial evento Subscrição com o restante desativado', () => {
        ...
        it('GET - Consultar evento capital que foi homologado parcial com o restante desativado', () => {
        ...
        it('GET - Consultar capital tipo unitário com um ativo após homologar parcial o evento Subscrição com o restante desativado', () => {
            ...
            valorUnitarioAposEventoHomologado = 11.6120218
        ...
        it('GET Histórico capital - Consultar no histórico os eventos capital já homologado', () => {
            ...
            cy.getCEM('Capital/' + idEmissor + '/history').then(resp => {
                ...
                buscaEventoSubscricao = resp.body.filter(i => i.eventType == "Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.id == idNovoCapitalAposEventoHomologado), "Id novo capital após evento homologado - Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.capital == valorCapitalAposEventoHomologado), "valor capital - Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.eventType == "Subscrição"), "Tipo evento capital - Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.quantity == qtdesAtivosOrdinarioTotalAposEventoHomologado), "Quantidade total ativos - Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.approvalDate.includes(dataHoje)), "Data aprovacao capital - Subscrição")
                assert.isTrue(buscaEventoSubscricao.some(i => i.startDate == dataSeguinte), "Data inicio vigencia capital - Subscrição")
