...
let idTipoEventoCapital: number = EnumTipoEventoCapital.ajuste
...
let buscaEventoAjuste: any
let dataSeguinte: string
let buscaListaAtivoOrdinario: any
let buscaListaAtivoPreferencial: any

describe('Emissor BDR - Homologar completo evento Ajuste - Capital Tipo Nominal', () => {
    ...
    capitalEventoEmissor.forEach((capitalData: CemModel) => {
        ...
        it('GET - Consultar capital tipo nominal com vários ativos já cadastrado com o evento IPO já homologado', function () {
        ...
        it('GET Histórico capital - Consultar no histórico o capital antes de homologar completo o evento Ajuste', () => {
        ...
        it('POST - Cadastrar evento Ajuste pendente de homologação', () => {

            statusEventoCapital = false
            qtdeAtivoPreferencialAlterar = 2947
            qtdeAtivoOrdinarioAlterar = 3874
            justificativaEvento = "Ajustes Quantidades Ativos"
        ...
        it('GET lista eventos pendente de homologação - Consultar evento capital cadastrado pendente de homologação', () => {
            ...
            cy.getCEM('capitalEvent/' + idEmissor + '/homologate-list').then(resp => {
                ...
                expect(resp.body[0]).to.have.property("capitalEventTypeDescription", 'Ajuste')
                ...
                expect(resp.body[0].capitalType).to.have.property("description", 'Ajuste')
                expect(resp.body[0].capitalType).to.have.property("fieldsCode", 2)
                expect(resp.body[0].capitalType).to.have.property("id", EnumTipoEventoCapital.ajuste)
        ...
        it('PUT - Homologar completo evento Ajuste', () => {
        ...
        it('GET - Consultar capital tipo unitário com vários ativos após homologar completo evento Ajuste', function () {
        ...
        it('GET Histórico capital - Consultar no histórico os eventos capital já homologado', () => {
            ...
            cy.getCEM('Capital/' + idEmissor + '/history').then(resp => {
                ...
                buscaEventoAjuste = resp.body.filter(i => i.eventType == "Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.id == idNovoCapitalAposEventoHomologado), "Id novo capital após evento homologado - Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.capital == valorPrecoCapital), "valor capital - Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.eventType == "Ajuste"), "Tipo evento capital - Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.quantity == qtdeTotalAtivosAposEventoHomologado), "Qtdes ativos total capital - Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.approvalDate.includes(dataHoje)), "Data aprovacao capital - Ajuste")
                assert.isTrue(buscaEventoAjuste.some(i => i.startDate == dataSeguinte), "Data inicio vigencia capital - Ajuste")
