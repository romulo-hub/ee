...
describe('Tipo emissor BDR NP com capital Tipo Nominal com um ativo cadastrado - Na grid eventos a homologar e homologar eventos pendentes - Validar as quantidades ativos capital após cancelar a edição do evento sem homologar', () => {
  ...
  capitalEmissor.forEach((capitalData: CemModel) => {
    ...
    beforeEach(() => {
      ...
      capitalNovoEventoUtilitario.emissorSemCapitalSocialNovoEventoSemHomologar().criarEventoConversaoAcoesComAddNovoAtivoCapitalAHomologar(capitalData.EventoCapital)
    ...
    it("Na grid EVENTOS A HOMOLOGAR validar as quantidades dos ativos após cancelar a edição do evento sem homologar", function () {
      ...
      capitalNovoEventoUtilitario.emissorSemCapitalSocialEditarEventoSemHomologar().cancelarAlteracoesNaEdicaoEventoConversaoAcoesUmAtivoCapital(capitalData.EventoCapital)
      cy.wait(200)
    ...
    it("Na homologação eventos pendentes validar as quantidades dos ativos após cancelar a edição do evento sem homologar", function () {
      ...
      cy.get('mat-table').get('[class="mat-table cdk-table"]').get('[class="mat-row cdk-row ng-star-inserted"]').within(() => {
        ...
        cy.get('[id="shareClassQuantityHomologated"]').should('not.have.value', '2.100')
