...
describe('Tipo emissor Cia Aberta com capital tipo nominal com varios ativos cadastrado - Cadastrar evento Redução de Capital sem homologar com exclusão dos ativos anteriores, adicionar novo ativo, homologar completo o evento Redução Capital, consultar a grid EVENTOS A HOMOLOGAR, validar os valores capital e histórico capital conforme o evento cadastrado', () => {
  ...
  capitalEmissor.forEach((capitalData: CemModel) => {
    ...
    before(() => {
      ...
      capitalNovoEventoUtilitario.capitalNovoEventoSemHomologar().criarEventoReducaoCapitalAHomologarExclusaoAtivosAnterioresAddNovoAtivo(capitalData.EventoCapital)
    ...
    it("Consultar a grid EVENTOS A HOMOLOGAR o evento Redução de Capital sem homologar", function () {
      ...
      capitalEventoPage.gridEventosAHomologarModal().valorCapitalEvento('[placeholder="Capital Final"]').should('have.value', 'R$ 22.562,00')
      ...
      cy.get('[class="mat-row cdk-row row-table ng-star-inserted"]').within(() => {
        ...
        cy.get('[id="shareClassQuantity"]').contains(' 1.252 ').should('be.visible')
      })
    })

    it("Homologar completo o evento Redução de Capital com exclusão dos ativo anteriores e novo ativo adicionado", () => {

      capitalNovoEventoUtilitario.homologarEventoPendentesCapital().homologarCompletoEventoReducaoCapitalExclusaoAtivosAnterioresAddNovoAtivo(capitalData.EventoCapital)
      capitalEventoPage.gridEventosAHomologarModal().fecharGridEventosAHomologar()

    })

    it("Validar as alterações no capital cadastrado após homologar completo o evento Redução de Capital", () => {

      capitalPage.capitalSocial().should('have.value', 'R$ 22.562,00').should('be.visible')
      cy.get('#resp-table-body').within(() => {

        capitalUtilitario.validarTagAlongGridAtivos(' PA ', '82%')
        capitalPage.novoAtivoModal().quantidadeAlterarCapital().contains('1.252').should('not.be.enabled').should('be.visible')
      ...
      capitalPage.quantidadeAtivosPreferencial()
        ...
        .should('have.value', '1.252')
        .should('be.visible')

    })

    it("Validar os detalhes no histórico do capital após homologar completa o evento Redução de Capital com exclusão dos ativos anteriores e novo ativo adicionado", function () {
      ...
      cy.get('app-history-detail').get('[class="modal"]').within(() => {
        ...
        capitalPage.historicoCapitalModal().detalhesHistoricoCapitalModal().capitalSocialHistoricoCapital().should('be.visible').should('not.be.focused').should('have.value', 'R$ 22.562,00')
        ...
        capitalPage.historicoCapitalModal().detalhesHistoricoCapitalModal().quantidadeAtivosPreferencialHistoricoCapital().should('be.visible').should('not.be.focused').should('have.value', '1.252')

        cy.get('[class="mat-table cdk-table"]').within(() => {

          capitalPage.historicoCapitalModal().detalhesHistoricoCapitalModal().quantidadeHistoricoCapital().contains('1.252').should('not.be.enabled').should('be.visible')
      ...
      capitalPage.historicoCapitalModal().capitalSocialHistoricoCapital().should('not.be.enabled').should('be.visible').contains(' R$ 22.562,00 ')
