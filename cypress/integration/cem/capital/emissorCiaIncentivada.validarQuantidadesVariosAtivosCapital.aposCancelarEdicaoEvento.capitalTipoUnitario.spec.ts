...
describe('Tipo emissor Cia Incentivada com capital Tipo Unitário com vários ativos cadastrado - Na grid eventos a homologar e homologar eventos pendentes - Validar as quantidades ativos capital após cancelar a edição do evento sem homologar', () => {
  ...
  capitalEmissor.forEach((capitalData: CemModel) => {
    ...
    it("Na grid EVENTOS A HOMOLOGAR validar as quantidades dos ativos após cancelar a edição do evento sem homologar", function () {
      ...
      capitalNovoEventoUtilitario.emissorSemCapitalSocialEditarEventoSemHomologar().cancelarAlteracoesNaEdicaoEventoConversaoAcoesVariosAtivosCapital(capitalData.EventoCapital)
      cy.wait(200)
