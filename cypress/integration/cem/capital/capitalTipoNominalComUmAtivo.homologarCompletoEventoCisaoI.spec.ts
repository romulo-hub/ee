...
describe('Tipo emissor Cia Aberta com capital tipo nominal com um ativo cadastrado - Homologar completo o evento Cisão I, consultar a grid EVENTOS A HOMOLOGAR e validar as alterações do evento homologado no histórico e capital já cadastrado', () => {
    ...
    capitalEmissor.forEach((capitalData: CemModel) => {
        ...
        before(() => {
            ...
            capitalNovoEventoUtilitario.capitalNovoEventoSemHomologar().criarEventoSobrescreverValorCapitalAHomologar(capitalData.EventoCapital)
        ...
        it("Homologar completo o evento Cisão I e consultar a grid EVENTOS A HOMOLOGAR", function () {

            capitalNovoEventoUtilitario.homologarEventoPendentesCapital().homologarCompletoEventoCisaoINominalComUmAtivo(capitalData.EventoCapital)
            capitalEventoPage.gridEventosAHomologarModal().fecharGridEventosAHomologar()

        })

        it("Validar as alterações do evento Cisão I homologado completo no capital cadastrado", () => {
        ...
        it("Validar os detalhes no histórico do capital após confirmar homologação completa do evento Cisão I", () => {
            ...
            cy.get('app-history-detail').get('[class="modal"]').within(() => {
                ...
                capitalPage.historicoCapitalModal().detalhesHistoricoCapitalModal().dataAprovacaoHistoricoCapital().should('be.visible').should('not.be.focused')
                capitalPage.historicoCapitalModal().detalhesHistoricoCapitalModal().dataHomologacaoHistoricoCapital().should('be.visible').should('not.be.focused')
            ...
            capitalPage.historicoCapitalModal().dataAprovacaoHistoricoCapital().should('not.be.enabled').should('be.visible')
            capitalPage.historicoCapitalModal().inicioVigenciaHistoricoCapital().should('not.be.enabled').should('be.visible')
