...
describe('Tipo emissor Cia Incentivada com capital tipo unitário com um ativo cadastrado - Validar a mensagem de não permitir Homologar parcial com o manter restante desativado do evento Desdobramento com emissor sem capital social, consultar a grid EVENTOS A HOMOLOGAR e validar o capital cadastrado sem nenhuma alteração do evento Desdobramento que não foi homologado parcial', () => {
    ...
    capitalEmissor.forEach((capitalData: CemModel) => {
        ...
        it("Homologar parcial o evento Desdobramento com o manter restante desativado e consultar a grid EVENTOS A HOMOLOGAR", function () {

            capitalNovoEventoUtilitario.emissorSemCapitalSocialHomologarEventoPendentesCapital().homologarParcialEventoDesdobramentoUnitarioComUmAtivo(capitalData.EventoCapital)
            ...
            capitalEventoPage.gridEventosAHomologarModal().valorCapitalEvento('[placeholder="Montante"]').should('have.value', 'R$ 0,00')
        ...
        it("Validar o capital cadastrado sem nenhuma alteração do evento Desdobramento que não foi homologado parcial", () => {
