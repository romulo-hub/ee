...
describe('Tipo emissor Cia Aberta com capital Tipo Nominal com um ativo cadastrado - Na grid eventos a homologar e homologar eventos pendentes: Validar as quantidades ativos capital após cancelar a edição do evento sem homologar', () => {
  ...
  capitalEmissor.forEach((capitalData: CemModel) => {
    ...
    beforeEach(() => {
      ...
      capitalNovoEventoUtilitario.capitalNovoEventoSemHomologar().criarEventoConversaoAcoesCapitalAHomologarComAddNovoAtivoTipoNominalUmAtivo(capitalData.EventoCapital)
    ...
    it("Na grid EVENTOS A HOMOLOGAR validar as quantidades dos ativos após cancelar a edição do evento sem homologar", function () {
      ...
      capitalNovoEventoUtilitario.capitalEditarEventoSemHomologar().cancelarAlteracoesNaEdicaoEventoConversaoAcoesUmAtivoCapital(capitalData.EventoCapital)
      cy.wait(200)
